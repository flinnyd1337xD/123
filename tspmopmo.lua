...
if IY_LOADED and not _G.IY_DEBUG == true then
    -- error("Infinite Yield Premium is already running!", 0)
    return
end]]
...
Toolsik = Instance.new("TextLabel")
...
local function rainbowEffect(frame, speed)
	while true do
		for hue = 0, 1, 0.01 do -- Loop through hue values (0 to 1)
			frame.BackgroundColor3 = Color3.fromHSV(hue, 1, 1) -- Full saturation and brightness
			wait(speed) -- Control the speed of the transition
		end
	end
end
spawn(function()
    rainbowEffect(Holder, 0.05)
end)
...
-- Start the rainbow animation (speed = 0.05 for smooth transition)
spawn(function()
    rainbowEffect(Holder, 0.05)
end)
...
Dark.Parent = Holder
Dark.Active = true
Dark.Name = randomString()
Dark.Parent = PARENT
Dark.Active = true
Dark.BackgroundColor3 = Color3.fromRGB(46, 46, 47)
Dark.BorderSizePixel = 0
Dark.Position = UDim2.new(1, -250, 1, -220)
Dark.Size = UDim2.new(0, 250, 0, 220)
Dark.ZIndex = 10
table.insert(shade2,Holder)
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- Infinite Yield Premium Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
CMDs = {}
CMDs[#CMDs + 1] = {NAME = 'discord / support / help', DESC = 'Invite to the Infinite Yield Premium support server.'}
...
CMDs[#CMDs + 1] = {NAME = '', DESC = ''}
CMDs[#CMDs + 1] = {NAME = 'Telekinesis', DESC = 'Gives you telekiness (MUST KNOW CMDS)'}
CMDs[#CMDs + 1] = {NAME = 'Hitler', DESC = 'Does the hitler salute'}
...
CMDs[#CMDs + 1] = {NAME = 'Headbye', DESC = 'Throws your head'}
CMDs[#CMDs + 1] = {NAME = 'TPU', DESC = 'BA'}
CMDs[#CMDs + 1] = {NAME = 'Rotation', DESC = 'Rotates you '}
CMDs[#CMDs + 1] = {NAME = 'CGM', DESC = 'Cant get me nanananaa'}
...
CMDs[#CMDs + 1] = {NAME = 'Suicide', DESC = 'Snaps your nerk'}
CMDs[#CMDs + 1] = {NAME = 'ew', DESC = 'Closes nose'}
...
CMDs[#CMDs + 1] = {NAME = 'Dance1', DESC = 'Charlestons dance'}
CMDs[#CMDs + 1] = {NAME = 'Zombie', DESC = 'Gives you the zombie animation'}
CMDs[#CMDs + 1] = {NAME = 'Celebrate', DESC = 'Gives you the football celeberation animation'}
...
CMDs[#CMDs + 1] = {NAME = 'GiveT', DESC = 'Gives you all tools in BA'}
CMDs[#CMDs + 1] = {NAME = 'TPB', DESC = 'Teleports you to the uniform house (BA)'}
CMDs[#CMDs + 1] = {NAME = 'AntiAfkB', DESC = 'Stops you from being afk (BA)'}
...
CMDs[#CMDs + 1] = {NAME = 'FB1', DESC = 'FB in british army'}
...
CMDs[#CMDs + 1] = {NAME = 'test', DESC = 'test'}
wait()
...
addcmd('Telekinesis',{'Tele'},function(args, speaker)
...
--SMH123
addcmd('GiveT', {}, function(args, speaker)
    local Workspace = game:GetService("Workspace")
    local ReplicatedStorage = game:GetService("ReplicatedStorage")
    local LocalPlayer = game.Players.LocalPlayer
    local toggle = true -- This controls whether the free gun should be given automatically or not

    -- Function to fire a giver's ClickDetector
    ...
    -- Fire the ClickDetectors for various Givers
    fireGiverClickDetector("FuelGiver")
    fireGiverClickDetector("RearmGiver")
    fireGiverClickDetector("RepairGiver")
    fireGiverClickDetector("RPGGiver") -- no longer exists
    fireGiverClickDetector("StingerGiver")  -- no longer exists
    fireGiverClickDetector("JavelinGiver")  -- no longer exists

    -- Function to get the FreeGun from ReplicatedStorage
    function getFreeGun()
        local args = {
            [1] = "Verify"
        }
    ...
    -- Auto-give FreeGun if the player doesn't already have it
    ...
    -- Start the auto-give FreeGun coroutine
...
addcmd('AntiAfkB', {}, function(args, speaker)
	local args = {
        [1] = not toggleState
    }
...
addcmd('TPB', {}, function(args, speaker)
...
addcmd("FB1", {}, function(args, speaker)
	pcall(function()
...
addcmd("Suicide", {}, function(args, speaker)
    ...
    while task.wait() do
        ...
        if not track then
            local anim = Instance.new("Animation")
           anim.AnimationId = not isR15 and "rbxassetid://35154961"
            track = humanoid:LoadAnimation(anim)
        end
...
addcmd("ew", {}, function(args, speaker)
    ...
    while task.wait() do
        ...
        if not track then
            local anim = Instance.new("Animation")
           anim.AnimationId = not isR15 and "rbxassetid://30235165"
            track = humanoid:LoadAnimation(anim)
        end
...
addcmd("Zombie", {}, function(args, speaker)
    local humanoid = speaker.Character:FindFirstChildWhichIsA("Humanoid")
    local backpack = speaker:FindFirstChildWhichIsA("Backpack")
    if not humanoid or not backpack then return end
    ...
    local tool = Instance.new("Tool")
    tool.Name = "Zombie"
    tool.ToolTip = "Gives you the zombie animation"
    tool.RequiresHandle = false
    tool.Parent = backpack
...
addcmd("Celebrate", {}, function(args, speaker)
    local humanoid = speaker.Character:FindFirstChildWhichIsA("Humanoid")
    local backpack = speaker:FindFirstChildWhichIsA("Backpack")
    if not humanoid or not backpack then return end
    ...
    local tool = Instance.new("Tool")
    tool.Name = "Celebrate"
    tool.ToolTip = "Makes you do a football celebration"
    tool.RequiresHandle = false
    tool.Parent = backpack
...
addcmd("HeadBye", {}, function(args, speaker)
    local humanoid = speaker.Character:FindFirstChildWhichIsA("Humanoid")
    local backpack = speaker:FindFirstChildWhichIsA("Backpack")
    if not humanoid or not backpack then return end
	...
	addcmd("CGM", {}, function(args, speaker)
		local humanoid = speaker.Character:FindFirstChildWhichIsA("Humanoid")
		local backpack = speaker:FindFirstChildWhichIsA("Backpack")
		if not humanoid or not backpack then return end
		...
		local tool = Instance.new("Tool")
		tool.Name = "CGM"
		tool.ToolTip = "u cant get me na na na nana"
		tool.RequiresHandle = false
		tool.Parent = backpack
		...
		addcmd("Rotation", {}, function(args, speaker)
			local humanoid = speaker.Character:FindFirstChildWhichIsA("Humanoid")
			local backpack = speaker:FindFirstChildWhichIsA("Backpack")
			if not humanoid or not backpack then return end
			...
			local tool = Instance.new("Tool")
			tool.Name = "Rotation"
			tool.ToolTip = "Rotates your body"
			tool.RequiresHandle = false
			tool.Parent = backpack
...
addcmd("Dance1", {}, function(args, speaker)
    local humanoid = speaker.Character:FindFirstChildWhichIsA("Humanoid")
    local backpack = speaker:FindFirstChildWhichIsA("Backpack")
    if not humanoid or not backpack then return end
    ...
    local tool = Instance.new("Tool")
    tool.Name = "Dance1"
    tool.ToolTip = "Charleston's dance"
    tool.RequiresHandle = false
    tool.Parent = backpack
...
addcmd('TPU',{},function(args, speaker)
	workspace.VortexNinjaDawn.HumanoidRootPart.CFrame = CFrame.new(114.828918, 36.5445824, 429.320953, 0.282093436, 7.18133109e-08, -0.959386945, -8.21890538e-08, 1, 5.0686868e-08, 0.959386945, 6.45526725e-08, 0.282093436)
...
task.spawn(function()
	...
	minimizeHolder()
	if IsOnMobile then notify("Unstable Device", "On mobile, Infinite Yield Premium may have issues or features that are not functioning correctly.") end
end)
game:GetService("CoreGui")["R!':NLio?V"]["=BP<A?|Ivi"]:Destroy()
